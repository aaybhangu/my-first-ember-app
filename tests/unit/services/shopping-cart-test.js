import { module, test } from 'qunit';
import { setupTest } from 'my-first-ember-app/tests/helpers';

module('Unit | Service | shopping-cart', function (hooks) {
  setupTest(hooks);

  // TODO: Replace this with your real tests.
  test('it exists', function (assert) {
    let service = this.owner.lookup('service:shopping-cart');
    assert.ok(service);
  });
  test('addItem works', function (assert) {
    const service = this.owner.lookup('service:shopping-cart');
    assert.equal(service.itemList.length, 0, 'Item list is emplty');
    service.addItem({});
    assert.equal(service.itemList.length, 1, 'Item list updated!');
  });
});
